...
Diagramma neuzrāda acīmredzamas sistemātiskas kļūdas (piemēram, izliekumu vai līkni), tačuu atlikumu lielums norāda, kair diezgan plaša atlikumu izkliede ap nulli, kas varētu norādī uz neprecizitāti. Kā arī modelis izskaidro tikai nelielu datu variāciju. Atlikumi neatbilst precīzi normālam sadalījumam.
...
gg_diagnose(model10, ncol = 2) 

```

Regresijas modelis izskaidro tikai 3.8% no vērtību variēšanas, modelis kopumā ir būtisks (F(983;21)=2.889, p < 0.0001). Taču atsevišķu faktoru ietekmes nav statistiski nozīmīgas. Pēc modeļa diagnostikas novērots, ka atlikumu vērtības neizskatās normāli sadalītas (lielo vērtību astē), ir ietekmīgās vērtības.
...
Regresijas modelis izskaidro tikai 3.8% no vērtību variēšanas, modelis kopumā ir būtisks (F(983;21)=2.889, p < 0.0001). Taču atsevišķu faktoru ietekmes nav statistiski nozīmīgas.
...
ggplot(cukur, aes(x = Area, y = Tonn_Hect)) +
  ...
  labs(
    title = "Attiecība starp platību un ražu",
    x = "Platība (Area)",                      
    y = "Raža (Tonnas/Hektārs)"                
  )
...
Pec AIC vertıbas vienkāršākais modelis (1. modelis) ir labāks, jo šı modeļa AICc vertıba ir  mazaka neka 2. modeļa.
